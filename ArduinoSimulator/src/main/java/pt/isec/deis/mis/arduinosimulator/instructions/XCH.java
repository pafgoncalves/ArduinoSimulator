 ...
 */
public class XCH extends BaseInstruction {
    
    @Override
    public void execute(CPU cpu, int rdAddr, int op2) {

        int addr = (cpu.getSRAM().getRegisterRAMPZ()<<16) | cpu.getSRAM().getRegisterZ();
        int tmp = cpu.getSRAM().get(addr);
        cpu.getSRAM().set(addr, cpu.getSRAM().getRegister(rdAddr));
        cpu.getSRAM().setRegister(rdAddr, tmp);
