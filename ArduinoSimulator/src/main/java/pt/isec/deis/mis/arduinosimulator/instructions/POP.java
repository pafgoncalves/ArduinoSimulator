 ...
 */
public class POP extends BaseInstruction {
    
    @Override
    public void execute(CPU cpu, int rdAddr, int op2) {
        cpu.getSRAM().setStackPointer(cpu.getSRAM().getStackPointer()+1);
        int rd = cpu.getSRAM().getStackW(cpu.getSRAM().getStackPointer());
        cpu.getSRAM().setRegister(rdAddr, rd);
        cpu.incPc();
